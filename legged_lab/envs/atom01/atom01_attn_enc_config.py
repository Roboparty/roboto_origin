...
def mirror_perception_observation(perception_obs):
    batch_size = perception_obs.shape[0]
    obs_flat = perception_obs.view(batch_size, -1, 3)  # [batch, 187, 3]
    mirrored_obs = obs_flat[:, map_scan_mirror_indices, :]  # mirror indices
    map_scan_mirror_signs_tensor = torch.tensor(map_scan_mirror_signs, dtype=perception_obs.dtype, device=perception_obs.device)
    map_scan_mirror_signs_tensor = map_scan_mirror_signs_tensor.view(1, -1, 3)  # [1, 187, 3]
    mirrored_obs = mirrored_obs * map_scan_mirror_signs_tensor
    mirrored_obs = mirrored_obs.view(batch_size, 17, 11, 3)
    return mirrored_obs
