...
class ATOM01AttnEncEnv(BaseEnv):
    ...
    def map_scan_base(self):
        ...
        height_scan = (
                    self.height_scanner.data.pos_w[:, :3].unsqueeze(1)
                    - self.height_scanner.data.ray_hits_w[..., :3]
                ) * self.obs_scales.height_scan
        height_scan[..., 2] -= self.cfg.normalization.height_scan_offset * self.obs_scales.height_scan
        # shape: [B, L*W, 3]
        L = grid_shape[0]
        W = grid_shape[1]
        ...
        # To get a grid of shape [L, W], we first view it as [B, W, L, 3] and then permute to [B, L, W, 3].
