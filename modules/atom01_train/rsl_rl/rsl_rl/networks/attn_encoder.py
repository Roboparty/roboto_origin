...
class AttentionEncoderBlock(nn.Module):
    def __init__(self, d_obs:int, embedding_dim=64, h=8, map_size=(17,11), map_resolution=0.1):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, padding="same", padding_mode='replicate'),
            nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=3, padding="same", padding_mode='replicate'),
            nn.ReLU(),
            nn.Conv2d(32, (self.embedding_dim - 3), kernel_size=3, padding="same", padding_mode='replicate'),
        )
    ...
    def forward(self, map_scans, proprioception):
        ...
        return map_encoding, attn_weights, cnn_features

class AttentionEncoder(nn.Module):
    def __init__(self, d_obs, embedding_dim=64, h=8, map_size=(17,11), map_resolution=0.1):
        super(AttentionEncoder, self).__init__()

        # 注意力地图编码模块
        self.encoder = AttentionEncoderBlock(d_obs, embedding_dim, h, map_size, map_resolution)

    def forward(self, map_scans, proprioception, embedding_only:bool=False):
        # 获取编码
        map_encoding, attention, cnn_features= self.encoder(map_scans, proprioception)
        # [B,d], [B,d_obs], [B,W,L]
        # 拼接地图编码和原始本体感觉
        if embedding_only:
            return map_encoding, attention, cnn_features
        else:
            combined = torch.cat([map_encoding, proprioception], dim=-1)  # (B, d + d_obs)
            return combined, attention, cnn_features
...
if __name__ == "__main__":
    ...
    # 前向传播
    embedding, attention, _ = model(map_scans, proprioception, True)
